...
- No new request load without a budget line: `--once` reads a cache entry younger than `poll::SCOREBOARD_LIVE` (15 s) without the network, so a status bar polling every 30 s costs at most the nine scoreboards the TUI would have fetched in that window anyway.
