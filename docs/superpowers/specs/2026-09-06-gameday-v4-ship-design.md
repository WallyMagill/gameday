...
- The GIF: `docs/demo.tape` (the sitting's cut a, `docs/demo-a.tape` renamed, `docs/demo-b.tape`'s board→tv cut deleted) recorded with `vhs` 0.11.0 (`brew "vhs"`, installed for this wave) — board → the red-zone beat → the scoring cut → zoom, 120x36 cells, broadcast theme, ~24 s, `docs/demo.gif` 449 KB. `cargo package --list | grep -c demo` is 0 (`docs/` and `*.tape` are excluded from the crate).
