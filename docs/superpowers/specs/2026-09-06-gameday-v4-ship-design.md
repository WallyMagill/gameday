...
- NFL week-one window (§8.5), Thursday 2026-09-10, SF at LAR (event `401872657`), run from 21:37 ET with the game live in the second quarter:
  ...
  - **Request budget:** `GAMEDAY_LOG_REQUESTS=1` on the measured session (NFL only): one `Scoreboard(Nfl)` line every ~15 s, 281 requests by 22:48 ET (71 minutes) — 3.96 a minute, scoreboards only, no summary fetched (the zoom was never opened; the scoring cuts came from the scoreboard's own last play). The final count lands with the FINAL notification below.
