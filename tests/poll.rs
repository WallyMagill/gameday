...
fn scoreboards_are_staggered_not_burst() {
    ...
    assert_eq!(
        first.len(),
        1,
        "one league per tick on a cold start, got {first:?}"
    );
...
fn a_cold_start_spreads_its_first_pass_over_cold_spread_not_the_idle_minute() {
    ...
    for l in League::ALL {
        assert!(
            fired.contains(&l),
            "{} never fired inside COLD_SPREAD: {fired:?}",
            l.slug()
        );
    }
...
fn a_backed_off_league_is_not_clamped_when_the_cadence_shrinks() {
    ...
    assert!(
        before > SCOREBOARD_LIVE,
        "backoff should be minutes, got {before:?}"
    );
    w.any_live = true;
    ...
    assert_eq!(
        before, after,
        "a live cadence must not pull a backed-off retry forward"
    );
}
...
fn summary_and_stats_only_for_the_zoomed_game() {
    ...
    let reqs: Vec<Request> = (0..200)
        .flat_map(|i| s.due(&w, false, t0 + Duration::from_millis(200 * i)))
        .collect();
    assert!(
        !reqs
            .iter()
            .any(|r| matches!(r, Request::Summary(..) | Request::Stats(..))),
        "{reqs:?}"
    );
    ...
    assert!(
        reqs.contains(&Request::Stats(League::Mlb, "401".into())),
        "a fresh zoom fetches stats at once"
    );
}
...
fn ten_minutes_of_nine_live_leagues_and_one_zoom_stays_under_budget() {
    ...
    assert!(
        n <= 440,
        "{n} requests in 10 minutes, budget 420 (+5% jitter slack)"
    );
    assert!(
        n >= 380,
        "{n} — suspiciously few; the scheduler is starving something"
    );
}
...
fn a_failing_league_backs_off_alone_with_jitter_and_caps() {
    ...
    assert!(
        r1 >= BACKOFF_BASE * 80 / 100 && r1 <= BACKOFF_BASE * 120 / 100,
        "first retry ≈5s ±20%, got {r1:?}"
    );
    assert!(
        s.next_retry(League::Nba, t0).is_none(),
        "NBA is not backed off by NFL's failure"
    );
    for _ in 0..10 {
        s.report(&Request::Scoreboard(League::Nfl), false, t0);
    }
    ...
    assert!(
        s.next_retry(League::Nfl, t0).is_none(),
...
fn refresh_now_makes_every_scoreboard_due_at_once() {
    ...
    assert_eq!(
        reqs.iter()
            .filter(|r| matches!(r, Request::Scoreboard(_)))
            .count(),
        3
    );
}
...
fn idle_cadence_is_slow_and_dated_and_standings_fire_on_target_change() {
    ...
    let _ = s.due(&w, false, t0);
    assert!(
        s.due(&w, false, t0 + Duration::from_secs(30)).is_empty(),
        "idle: nothing due at 30s"
    );
    assert_eq!(
        s.due(&w, false, t0 + Duration::from_secs(61)),
        vec![Request::Scoreboard(League::Cfb)]
    );
    ...
    assert!(
        s.due(&w, false, t0 + Duration::from_secs(63)).is_empty(),
        "same targets don't refire inside their windows"
    );
}
