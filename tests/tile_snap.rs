...
fn standard_tile_shows_score_and_live() {
    ...
    t.draw(|f| render_tile(f, f.area(), &live_kc(), Density::Standard, true, TileFx::default(), ScoreStyle::Compact)).unwrap();
    let s = buf_text(&t);
...
fn compact_tile_has_no_last_play_text() {
    ...
    t.draw(|f| render_tile(f, f.area(), &live_kc(), Density::Compact, false, TileFx::default(), ScoreStyle::Compact)).unwrap();
    let s = buf_text(&t);
    assert!(s.contains("27"), "{s}");
...
fn full_tile_includes_last_play_and_situation() {
    ...
    t.draw(|f| render_tile(f, f.area(), &live_kc(), Density::Full, false, TileFx::default(), ScoreStyle::Compact)).unwrap();
    let s = buf_text(&t);
    assert!(s.contains("Kelce"), "{s}");
